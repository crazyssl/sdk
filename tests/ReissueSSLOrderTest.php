<?php
namespace Crazyssl\Tests;

use Crazyssl\Tests\Abstracts\AbstractTest;

class ReissueSSLOrderTest extends AbstractTest
{
    /**
     * 重签测试
     *
     * @return void
     */
    public function testReissueSSLOrder()
    {
        /**
         * @var \Crazyssl\Client $sdk
         */
        $sdk = app('trustocean');

        $csr = "-----BEGIN CERTIFICATE REQUEST-----\\n-----END CERTIFICATE REQUEST-----";
        $email = 'test@test.com';

        $arguments = [1, $csr, 'dns', 1, 'www.test.com,www.test2.com', false, null, null, null, null, null, null, null, null, null, null, null, null, null, null];
        call_user_func_array([$sdk->order_mock, 'reissueSSLOrder'], $arguments);
        $order = call_user_func_array([$sdk->order, 'reissueSSLOrder'], $arguments);

        $this->assertEquals('success', $order->status);
        $this->assertObjectHasAttribute('dcv_info', $order);
    }
}
